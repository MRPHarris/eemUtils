...
#' Read multiple eem files using eemR::eem_read. Essentially identical to that function, but with handling for multiple files.
#'
#' @param file One or more file names.
#' @param recursive logical. Should the listing recurse into directories?
#' @param import_function Either a character string or a user-defined function to
...
eem_read_mod <- function (file,
                          recursive = FALSE,
                          import_function){
  # functions are current as of 07/07/21
  eem_import_function_factory2 <- function(import_function) {
  ...
  eem_check_import_function2 <- function(f) {
    arguments <- formals(f)
  ...
  if(length(file) > 1){
    ...
    # check import fn
    f <- eem_import_function_factory2(import_function)
    eem_check_import_function2(f)
    # Loop for eem processing and addition to eemlist_add
    for(i in seq_along(it_list)){
      # get file for this iteration
      file_it <- file[i]
      # is it a directory?
      stopifnot(file.exists(file_it) | file.info(file_it)$isdir, is.logical(recursive),
      ...
      res <- lapply(res, eem) # this applies the eem constructor
      class(res) <- "eemlist"
    ...
    stopifnot(file.exists(file) | file.info(file)$isdir, is.logical(recursive),
              is.function(import_function) | is.character(import_function))
    f <- eem_import_function_factory2(import_function)
    eem_check_import_function2(f)
    ...
    res <- lapply(res, eem) # this applies the eem constructor
    class(res) <- "eemlist"
    res[unlist(lapply(res, is.null))] <- NULL # what's the point of this? applying
    return(res)
  }
}
